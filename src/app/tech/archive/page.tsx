...
import { TechUpdateCard, type TechUpdate } from "@/components/tech-update-card";
  ...
  const fetchData = useCallback(async (date: string) => {
    ...
    try {
      const [feedRes, dailyRes] = await Promise.all([
        fetch(`/api/tech?date=${date}&limit=100`),
        fetch(`/api/daily-tech?date=${date}`),
      ]);
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isToday && (
              <button
                onClick={jumpToToday}
                className="px-3 py-2 rounded-lg bg-breaking/10 text-breaking border border-breaking/20 hover:bg-breaking/20 transition-all text-xs font-medium"
              >
                Today
              </button>
            )}
                        ...
                        <span className="flex items-center gap-2">
                          <span
                            className={`font-mono w-8 ${
                              isSelected ? "text-breaking/80" : "text-muted"
                            }`}
                          >
                            {dayName}
                          </span>
                        ...
                        {hasData && (
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${
                              isSelected
                                ? "bg-breaking shadow-[0_0_8px_rgba(220,38,38,0.8)]"
                                : "bg-breaking/50"
                            }`}
                          />
                        )}
