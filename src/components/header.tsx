    ...
    <header className="sticky top-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-xl">
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          ...
          {/* Navigation */}
          <nav className="flex items-center gap-1 sm:gap-2 pl-2 sm:pl-6 border-l border-white/10">
            <Link
              href="/"
              className={cn(
        ...
        {/* Center: LIVE indicator — absolutely positioned to stay perfectly centered */}
        <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-full border border-breaking/30 bg-breaking/5 backdrop-blur-md shadow-[0_0_15px_rgba(220,38,38,0.1)]">
        ...
        {/* Right Matcher to balance flex justify-between if we rely on it, otherwise empty */}
        <div className="flex items-center md:hidden">
          {/* Mobile LIVE indicator */}
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-breaking/30 bg-breaking/5">
            <span className="relative flex h-2 w-2 shrink-0">
              <span className="animate-pulse-live absolute inline-flex h-full w-full rounded-full bg-breaking opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-breaking shadow-[0_0_8px_rgba(220,38,38,0.8)]"></span>
            </span>
            <span className="text-[10px] font-bold text-breaking uppercase tracking-widest font-mono">
              Live
            </span>
          </div>
        </div>
